...
5. Класс (модель) Categories.
6. Класс (модель) CategoriesUsers.
...
2. Всплывает модальное окно с комментарием и ценой (поля не обязательны для заполнения).
3. После нажатия на кнопку "Отправить" в модальном окне, добавляется отклик на странице отображения задания.
...
7. Класс(модель) StatusTask. (Фактически меняется после статус в таблице "задачи", особо этот класс не учавствует,
   он используется только для отображения названия статуса) 
8. Класс(модель) ActionTask. (Меняет статус задачи)

### Процесс
1. На странице задания заказчик нажимает кнопку "Задание выполнено" и заполняет форму: оценка, комментарий.
2. Изменяется статус задачи на "Завершено", добавляется отзыв с оценкой и комментарием.
3. Отправляется уведомление исполнителю.
4. Переадресация на главную страницу(а лучше на "выполненные задания" у зазкачика).
---
...
4. Класс(бизнес-логика процессов) TaskService. Для всех процессов связанных с задачей: публикация и различные действия.
5. Класс(модель) NotificationUser.
6. Класс(модель) StatusTask.
7. Класс(модель) ActionTask.

### Процесс
1. Исполнитель нажимается на кнопку "Отказаться" в модальном окне на странице просмотра задания.
2. Меняется статус задания на "Провалено", у исполнителя(должно ли поле быть, или на основе задания вычислять)
   меняется увеличивается счётчик провалленых заданий.
3. Отправка уведомления заказчику.
4. Переадресация на страницу просмотра задания.
...
2. Поменять статус задачи "На исполнении".
3. Назначить автора отклика исполнителем.
4. Отправить уведомление исполнителю.
5. Передресовать на главную страницу.
...
1. Уведомление отправляются, если произошло следующе событие:
   ...
   - Старт задания
   - Завершение задания
2. Сформировать сообщение для электронной почты:
   - Тема (событие)
   - Текст(ссылка на задание, какой-нибудь текст в зависимости от события)
3. Проверить, что у исполнителя в настройках включено получение уведомлений этого типа.
...
2. Валидация поля сообщения, и существование id в таблице пользователей(непонятно, зачем проверять,
   если задание создано заказчиком, то в любом случае он есть в таблице).
